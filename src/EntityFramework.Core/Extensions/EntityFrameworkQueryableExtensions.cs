    ...
    public static class EntityFrameworkQueryableExtensions
    {
        ...
        internal static readonly MethodInfo ThenIncludeAfterCollectionMethodInfo
            = typeof(EntityFrameworkQueryableExtensions)
                ...
                .Single(mi => mi.GetParameters().Any(pi => pi.Name == "sourceCollection"));

        internal static readonly MethodInfo ThenIncludeAfterReferenceMethodInfo
            = typeof(EntityFrameworkQueryableExtensions)
                .GetTypeInfo().GetDeclaredMethods(nameof(EntityFrameworkQueryableExtensions.ThenInclude))
                .Single(mi => mi.GetParameters().Any(pi => pi.Name == "sourceReference"));

        // TODO API docs once we resolve #1709
        public static IIncludableQueryable<TEntity, TProperty> ThenInclude<TEntity, TPreviousProperty, TProperty>(
            [NotNull] this IIncludableQueryable<TEntity, ICollection<TPreviousProperty>> sourceCollection,
            [NotNull] Expression<Func<TPreviousProperty, TProperty>> navigationPropertyPath)
            where TEntity : class
        {
            return new IncludableQueryable<TEntity, TProperty>(
                sourceCollection.Provider.CreateQuery<TEntity>(
                    Expression.Call(
                        null,
                        ThenIncludeAfterCollectionMethodInfo.MakeGenericMethod(typeof(TEntity), typeof(TPreviousProperty), typeof(TProperty)),
                        new[] { sourceCollection.Expression, Expression.Quote(navigationPropertyPath) })));
        }

        public static IIncludableQueryable<TEntity, TProperty> ThenInclude<TEntity, TPreviousProperty, TProperty>(
            [NotNull]this IIncludableQueryable<TEntity, TPreviousProperty> sourceReference,
            [NotNull]Expression<Func<TPreviousProperty, TProperty>> navigationPropertyPath)
            where TEntity : class
        {
            return new IncludableQueryable<TEntity, TProperty>(
                sourceReference.Provider.CreateQuery<TEntity>(
                    Expression.Call(
                        null,
                        ThenIncludeAfterReferenceMethodInfo.MakeGenericMethod(typeof(TEntity), typeof(TPreviousProperty), typeof(TProperty)),
                        new[] { sourceReference.Expression, Expression.Quote(navigationPropertyPath) })));
        }
